...
describe('MultiSelectBar', () => {
  ...
  describe('styling', () => {
    it('applies bar wrapper classes', () => {
      const { container } = setup();
      const bar = container.querySelector('[data-test-id="multi-select-bar"]');
      expect(bar).toHaveClass(
        ...
        'tw:px-4',
        'tw:py-2',
        'tw:bg-[var(--color-surface-raised)]',
        'tw:border',
        'tw:border-[var(--color-border)]',
        'tw:rounded-[var(--radius-lg)]',
        'tw:shadow-[var(--shadow-md)]',
      );
    });

    it('applies close button classes', () => {
      const { container } = setup();
      const closeBtn = container.querySelector('[data-test-id="multi-select-close"]');
    ...
    it('applies count span classes', () => {
      const { container } = setup();
      const count = container.querySelector('[data-test-id="multi-select-count"]');
      expect(count).toHaveClass(
        'tw:text-sm',
        'tw:font-medium',
        'tw:text-[var(--color-text-primary)]',
        'tw:whitespace-nowrap',
      );
    });

    it('applies actions container classes', () => {
      const { container } = setup();
      const bar = container.querySelector('[data-test-id="multi-select-bar"]');
      const actionsContainer = bar?.querySelector('div:last-of-type') as HTMLElement;
      expect(actionsContainer).toHaveClass('tw:flex', 'tw:items-center', 'tw:gap-1', 'tw:ml-auto');
    });
  });
});
